...
export const AudioProvider = ({ children, driveToken, userId, onTokenRefresh }) => {
  ...
  // Back to a single, stable audio element
  ...
  const playTrackUrl = useCallback(async (track) => {
    ...
    try {
      ...
      // 1. CRITICAL: Set Metadata FIRST. 
      // This tells Android "Hey, we are still playing, here is the next song" 
      // BEFORE we wipe out the audio source.
      ...
      // 2. Ignore the native 'pause' event caused by src swapping
      isTransitioningRef.current = true;

      // 3. Swap the source and play
      ...
      if (playPromise !== undefined) {
        playPromise
          .then(() => {
            ...
            // Force OS to recognize active playback
            if ('mediaSession' in navigator) {
  ...
  useEffect(() => {
    ...
    const handlePause = () => { 
      ...
      // Ignore pauses caused by track transitions or the track naturally ending
      if (isNaturalEnd || isTransitioningRef.current) return; 
