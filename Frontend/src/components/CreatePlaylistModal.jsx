...
// FIX: Removed 'export' to make Vite Fast Refresh happy
...
const CreatePlaylistModal = ({ isOpen, onClose, onCreate, existingPlaylists }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const randomCover = PLAYLIST_COVERS[Math.floor(Math.random() * PLAYLIST_COVERS.length)];
      setSelectedCover(randomCover);
    }
  }, [isOpen]);
  ...
  const handleSubmit = (e) => {
    ...
    if (existingPlaylists.some(p => p.name.toLowerCase() === cleanName.toLowerCase())) {
    ...
    onCreate(cleanName, selectedCover);
  };

  return (
    // FIX: z-[200] to match other modals perfectly
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[200] flex items-end md:items-center justify-center p-0 md:p-4">
      
      {/* FIX: max-w-full on mobile ensures edge-to-edge fit, md:max-w-md keeps it compact on desktop */}
      <div className="bg-[#121826] border border-white/10 p-6 pb-8 md:p-8 rounded-t-3xl md:rounded-3xl w-full max-w-full md:max-w-md shadow-2xl animate-fade-in">
        ...
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl md:text-2xl font-bold font-display">New Playlist</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8">
          ...
          <div>
            <input 
              ...
              className={`w-full bg-white/5 border ${error ? 'border-rose-500' : 'border-white/10'} rounded-xl py-3 px-4 text-base text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50`}
              autoFocus
